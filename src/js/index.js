...
document.querySelectorAll(".bottom-nav.has-list").forEach((item) => {
  item.addEventListener("click", ({ target, parentElement }) => {
    let ele = target;
    // let ele = e.target;
    ...
    document.querySelectorAll(".bottom-nav.has-list").forEach((el) => {
      if (el !== ele) {
        ...
        el.querySelector(".bottom-nav__sublist-wrapper").classList.remove(
          "open"
        );
      }
    });
...
for (var i = 0; i < slides.length; i++) {
  slides[i].addEventListener("click", () => {
    ...
    let active_image = slides[i].querySelector("img");
    let active_date = slides[i].querySelector(".thumbnail-date");
    let active_link = slides[i].querySelector(".thumbnail__link");
...
function scrollToActivePlayer(item_class) {
  ...
  return [
    index * item_size - con.clientWidth / 2 + item_size * 0.5,
    activeItem,
  ];
}
...
const translateTo = function (e, right, container_class, item_class) {
  ...
  featuredPlayer.querySelector(".featured-player__link").src =
    container.querySelector(".active-player .player__link").src;
  featuredPlayer.querySelector(".featured-player__name").innerHTML =
    container.querySelector(".active-player .player__name").innerHTML;
  featuredPlayer.querySelector(".featured-player__position").innerHTML =
    container.querySelector(".active-player .player__position").innerHTML;
};

document
  .querySelector("#slide-players-next")
  .addEventListener("click", (e) =>
    translateTo(e, true, ".slides-container", ".player-item")
  );
document
  .querySelector("#slide-players-prev")
  .addEventListener("click", (e) =>
    translateTo(e, false, ".slides-container", ".player-item")
  );
...
playersContainer.addEventListener(
  "touchstart",
  (e) => (initailPosition = e.touches[0].clientX)
);
